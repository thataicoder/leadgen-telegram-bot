...
bot.on("callback_query", async (ctx)=>{
  ...
  if(session.step==="confirm"){
    ...
    if(data==="confirm_no"){
      reset(chatId);
      await ctx.editMessageText("Cancelled. Type /order to start again."); return;
    }
  }
  ctx.answerCbQuery();
});
...
bot.on("text", async (ctx)=>{
  ...
  if(session.step==="geo"){
    data.geo = text; session.step = "type";
  ...
  if(session.step==="qty"){
    data.quantity = text; session.step = "contact";
    return ctx.reply("Your email or Telegram @handle for follow-up?");
  }

  if(session.step==="contact"){
    data.contact = text; session.step = "notes";
...
async function submitOrder(ctx, data){
  const payload = {
    ...
    chat_id: ctx.chat?.id || "",
    geo: data.geo, type: data.type, quantity: data.quantity,
...
// keep a tiny health server for Render
const app = express();
